...
def make_dataset(image_list, labels):
    if labels:
      len_ = len(image_list)
      images = [(image_list[i].strip(), int(labels[i].strip())) for i in range(len_)]
    else:
...
class ImageList_idx(Dataset):
    ...
    def __getitem__(self, index):
        path, target = self.imgs[index]
        path_to_iamge = path.split("/")[-2:]
        path = f"/home/dsi/rotemnizhar/dev/noisy-temperature-scaling/visda-shot/data/validation/{path_to_iamge[0]}/{path_to_iamge[1]}"
        img = self.loader(path)
        if self.transform is not None:
            img = self.transform(img)
        if self.target_transform is not None:
